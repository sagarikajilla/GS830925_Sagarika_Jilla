...
import {
  ...
  Legend,
  ChartData,
} from 'chart.js'; // Import ChartData from chart.js here
import { Chart } from 'react-chartjs-2'; // Import Chart component from react-chartjs-2
...
const ChartPage: React.FC = () => {
  ...
  const labels = chartDataSample.map((rec) => rec.week);
  const gmDollars = chartDataSample.map((rec) => rec.gmDollars);
  const gmPercent = chartDataSample.map((rec) => rec.gmPercent);

  // Use ChartData from chart.js, specifying possible chart types
